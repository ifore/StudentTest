...
namespace WebApplication2.Controllers
{
   //[Authorize]
    public class StudentController : Controller
    {
        ...
        [HttpPost]
        public ActionResult UploadFiles(HttpPostedFileBase qqfile)
        {
            if (qqfile != null)
            {
                // this works for IE
                var filename = Path.Combine(Server.MapPath("~/UploadedFiles"), Path.GetFileName(qqfile.FileName));
            ...
            else
            {
                ...
                {
                    filename = Path.Combine(Server.MapPath("~/UploadedFiles"), Path.GetFileName(filename));
